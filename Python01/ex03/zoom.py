...
# Slicing used to "Zoom" the image both on the columns and rows

# np.dot used to make it grayscale, the :3 means it will ignore
# the 4th column, basically ignore the alfa in case of RGBA images

# The np.newaxis is to generate the 3rd column for printing purposes

# astype(np.uint8) is used to have the values in Integer instead of floats

# cmap is used to have it in gray scale otherwise the plt.imshow will
# think its a color image and it will have a green like color
